...
/**
 * Owner / admin marks attendance for an employee (no geofence).
 * status: PRESENT (set punch times) or LEAVE (clear punches).
 */
export async function markEmployeeAttendance(req, {
  userId,
  date,
  punchInAt,
  punchOutAt,
  ...
  if (!isAdminPanelRole(req.user?.role)) {
    throw httpError('Only business admins can mark employee attendance', 403);
  }

  const targetUserId = userId;
  if (!targetUserId) throw httpError('Employee is required');
  ...
  const todayKey = await businessTodayKey(req.businessId);
  if (dateKey > todayKey) throw httpError('Cannot mark attendance for a future date');
  ...
  const employee = await User.findOne({
    ...
    role: 'EMPLOYEE',
    status: { $ne: 'INACTIVE' }
  }).select('_id branchId name').lean();
  if (!employee) throw httpError('Employee not found', 404);

  if (req.branchScope !== 'all' && req.branchId) {
    if (employee.branchId && String(employee.branchId) !== String(req.branchId)) {
      throw httpError('Employee is not in this branch', 403);
    }
  }

  const markStatus = String(status || 'PRESENT').toUpperCase();
  if (!['PRESENT', 'LEAVE'].includes(markStatus)) {
    throw httpError('Status must be PRESENT or LEAVE');
  }

  let sessions = [];
  let dayStatus = markStatus === 'LEAVE' ? 'LEAVE' : 'CORRECTED';

  if (markStatus === 'PRESENT') {
    if (!punchInAt) throw httpError('Punch in time is required');
    const inAt = new Date(punchInAt);
    if (Number.isNaN(inAt.getTime())) throw httpError('Invalid punch in time');

    let outAt = null;
    if (punchOutAt) {
      outAt = new Date(punchOutAt);
      if (Number.isNaN(outAt.getTime())) throw httpError('Invalid punch out time');
      if (outAt <= inAt) throw httpError('Punch out must be after punch in');
    } else if (dateKey < todayKey) {
      throw httpError('Punch out time is required for past days');
    }
  ...
  const day = await AttendanceDay.findOneAndUpdate(
    ...
    {
      $set: {
        branchId: employee.branchId || req.branchId || null,
        status: dayStatus,
        source: 'ADMIN_CORRECTION',
        sessions,
        breaks: []
